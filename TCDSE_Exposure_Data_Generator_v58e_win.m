%% TCDSE Exposure Data Generator v5.8e
...
try 
    ...
    Com_Table = dist_table9pri2; %table9(1:height(land_use_types),2) .* double(table9(1:height(land_use_types),1)==0);
    if sum(Com_Table) ~= 0
    ...
    table9 = cell2mat(dist_table9);
    Ind_Table = dist_table9pri3; %table9(1:height(land_use_types),3) .* double(table9(1:height(land_use_types),1)==0);
    if sum(Ind_Table) ~= 0
...
for lut_indx = 1:length(land_use_types)
    if temp_numb_rescom(lut_indx)~=0
        ...
        [TV0_data_building(buildings_indicator(rescom_ids)).OccBld] = deal('ResCom') ;
    end
end
...
% Option-1
% Sum up all information for indivFacID_1 and indivFacID_2
