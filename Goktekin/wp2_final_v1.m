...
%% Input File Folder 
input_file_folder = 'C:\Kandilli Projects\tomorrow_cities\input files\';
% input_file_folder2 = 'C:\Kandilli Projects\tomorrow_cities\application\Input_Data\';
...
% file. Therefore new population will be obtained by using the newPop
% column in the Landuse layer dbf file. 

% For All Zone-IDs

user_percentage = 0.8;
nPeople = TV0_data_landuse.newPop * user_percentage;
...
for zone_idx = 1:size(TV0_data_landuse,1) % For every zone in Landuse Layer
    ...
    for i= 1:nHouse(zone_idx) 
        ...
        TV0_data_household(k,1).zoneID = TV0_data_landuse.id(zone_idx); % Passive, keep it for future purposes
        TV0_data_household(k,1).zoneType = TV0_data_landuse.type(zone_idx); % Passive, keep it for future purposes
    end
end
...
%         disp(['there are no populations in the zone id: ' num2str(TV0_data_landuse.id(zone_idx)) ] );
    ...
    temp_indx=[TV0_data_household(:).zoneID]==TV0_data_landuse.id(zone_idx);
...
for zone_idx = 1: size(TV0_data_landuse,1)
    zoneid = TV0_data_landuse.id(zone_idx);
    if nHouse(zone_idx)==0
        continue;
    end 
...
% 1 - 'agriculture'
% 2 - 'cityCentre'
% 3 - 'commercialResidential'
% 4 - 'highIncome'
% 5 - 'historicalPreservation'
% 6 - 'industry'
% 7 - 'lowIncomeA'
% 8 - 'lowIncomeB'
% 9 - 'midIncome'

land_use_types = {'agriculture','cityCentre','commercialResidential',...
    'highIncome','historicalPreservation','industry','lowIncomeA',...
    'lowIncomeB','midIncome'};
% Number of storeys
...
for zone_idx = 1: size(TV0_data_landuse,1) 
    zoneid = TV0_data_landuse.id(zone_idx);
    lut = TV0_data_landuse.type(zone_idx);
    lut_number = find(strcmp(land_use_types,lut));
    income_number = find(strcmp(avg_income_types,lut));
...
dist_table9 = {[1.0 0.0 0.0 0.0];... % LUT1 'agriculture'
               [0.0 0.0 1.0 0.0];... % LUT2 'cityCentre'
               [0.0 0.0 1.0 0.0];... % LUT3 'commercialResidential'
               [0.4 0.0 0.2 0.4];... % LUT4 'highIncome'
               [0.6 0.0 0.3 0.1];... % LUT5 'historicalPreservation'
               [0.0 1.0 0.0 0.0];... % LUT6 'industry'
               [0.7 0.0 0.1 0.2];... % LUT7 'lowIncomeA'
               [0.6 0.0 0.1 0.3];... % LUT8 'lowIncomeB'
               [0.5 0.0 0.4 0.1]};   % LUT9 'midIncome'         
...
for zone_idx = 1: size(TV0_data_landuse,1) 
    zoneid(zone_idx) = TV0_data_landuse.id(zone_idx);
    lut = TV0_data_landuse.type(zone_idx);
