%% TCDSE Exposure Data Generator v5.8b
...
% Version : 2 (updated as Version 3, 4 and 5 by Erdem Ozer lastly on 12 April 2023)
...
[dist_name,path_dist] = uigetfile('*.xlsx', 'Select Distribution Tables File');
...
if height(table_final) < 2^20
    writetable(table_final,fullfile(path_luf,"_outputs\layer_building.xlsx"));
else
    writetable(table_final,fullfile(path_luf,"_outputs\layer_building.csv"));
end

if height(household_data) < 2^20
    writetable(household_data,fullfile(path_luf,"_outputs\layer_household.xlsx"));
else
    writetable(household_data,fullfile(path_luf,"_outputs\layer_household.csv"));
end

if height(individual_data) < 2^20
    writetable(individual_data,fullfile(path_luf,'_outputs\layer_individual.xlsx'));
else
    writetable(individual_data,fullfile(path_luf,'_outputs\layer_individual.csv'));
end
...
if isequal(prj_luf.Name, "World_Cylindrical_Equal_Area") % change projection file from 4326 to 54034
    output_prj = fullfile(path_luf,"_outputs\"+extractBefore(bld_lyr_name,".")+"_building_centres"+".prj");
...
if isequal(prj_luf.Name, "World_Cylindrical_Equal_Area") % change projection file from 4326 to 54034
    output_prj = fullfile(path_luf,"_outputs\"+extractBefore(bld_lyr_name,".")+"_building_footprints"+".prj");
    fid2 = fopen(output_prj,'w');
    fprintf(fid2, 'PROJCS["World_Cylindrical_Equal_Area",GEOGCS["GCS_WGS_1984",DATUM["D_WGS_1984",SPHEROID["WGS_1984",6378137.0,298.257223563]],PRIMEM["Greenwich",0.0],UNIT["Degree",0.0174532925199433]],PROJECTION["Cylindrical_Equal_Area"],PARAMETER["False_Easting",0.0],PARAMETER["False_Northing",0.0],PARAMETER["Central_Meridian",0.0],PARAMETER["Standard_Parallel_1",0.0],UNIT["Meter",1.0]]');
    fclose(fid2);
end      
